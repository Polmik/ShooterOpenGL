...
void InitNetwork(ServerUDP& server, ClientUDP& client)
{
    ...
    if (!connectfile.is_open() || !(connectfile >> clientIp >> clientPort) || sf::IpAddress(clientIp) == sf::IpAddress::None)
    {
        ...
        clientIp = sf::IpAddress::LocalHost.toString();
        clientPort = defaultPort;
        std::ofstream connectOut("connect.txt", std::ofstream::out);
        connectOut << clientIp << std::endl << clientPort;
        connectOut.close();
    }
    connectfile.close();
    ...
    if (!connectfile.is_open() || !(connectfile >> serverPort))
    {
        ...
        std::ofstream serverOut("server.txt", std::ofstream::out);
        serverOut << serverPort;
        serverOut.close();
    }
    connectfile.close();

    if (sf::IpAddress::LocalHost == clientIp) { 
